...
function buildNano1Prompt(refinePrompt: string | null) {
  const basePrompt = [
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = buildNano1Prompt(refinePrompt);
    ...
    try {
      ...
      console.log(`REQ_NANO1_START ${requestId}`);
      ...
      console.log(`MODEL_DONE nano1 ${Date.now() - modelStartMs}ms`);
    ...
    if (!resultBase64) {
      return jsonError("Nano Banana 1 returned no image output.", 502, {
        model: "gemini-2.5-flash-image",
      });
    }
    ...
    return NextResponse.json(
      {
        ...
        output: { model: "gemini-2.5-flash-image" },
